...
async function autocomplete(val) {
    ...
    autoComplete.classList.add('search-autocomplete_show');
    for (let item of data) {
        let acItem = document.createElement('div');
        acItem.innerHTML = item;
        acItem.classList.add('search-autocomplete__item');
        autoComplete.append(acItem);
    }
}
...
document.querySelector('.search-btn').addEventListener('click', (e) => {
    ...
    downloadData(1, searchInput.value);
});
...
document.addEventListener("click", acListener );

function acListener(event){
    var element = event.target;
    if(element.classList.contains("search-autocomplete__item")){
        let autoComplete = document.querySelector('.search-autocomplete');
        let searchInput = document.querySelector('.search-field');
        searchInput.value = event.target.innerHTML;
        autoComplete.classList.remove('search-autocomplete_show');
        downloadData(1, searchInput.value);
    }
}